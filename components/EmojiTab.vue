...
function ensurePages() {
  ...
  if (selectedPageIndex.value >= config.value.emojiPages.length) {
    selectedPageIndex.value = Math.max(0, config.value.emojiPages.length - 1)
  }
}
...
function removePage(index: number) {
  ...
  selectedPageIndex.value = Math.min(selectedPageIndex.value, config.value.emojiPages.length - 1)
}
    ...
    <UCard>
      <template #header>
        <div class="flex items-center justify-between gap-3">
          <div>
            <h2 class="text-sm font-semibold">{{ $t('emoji.title') }}</h2>
      ...
      <div class="grid gap-4 lg:grid-cols-[minmax(0,1fr)_320px]">
        <div class="space-y-4">
          ...
          >
            <button
              ...
              :class="selectedKey === key ? 'border-primary ring-1 ring-primary/35' : 'border-(--ui-border-muted)'"
              @click="selectedKey = key"
            >
              <span class="text-3xl leading-none">{{ selectedPage.cells[key] || '＋' }}</span>
        ...
        <div v-if="selectedPage" class="space-y-4">
          <UCard>
            ...
            <div class="space-y-3">
              <div>
                <FieldLabel :label="$t('emoji.cellLabel', { key: selectedKey.toUpperCase() })" />
