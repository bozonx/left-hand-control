...
const pageCount = computed(() =>
    Math.max(
        1,
        Math.ceil(actions.value.length / pageSize),
        quickActionPages.value.length,
    ),
)
...
function createPageMeta(
    n: number = (config.value.quickActionPages?.length ?? 0) + 1,
) {
    return {
...
function ensurePages() {
    ...
    if (selectedPageIndex.value >= config.value.quickActionPages.length) {
        selectedPageIndex.value = Math.max(
            0,
            config.value.quickActionPages.length - 1,
        )
    }
}
...
function clampSelection() {
    ...
    if (
        ...
        const firstFilledIndex = actions.value.findIndex((action) =>
            action.action.trim(),
        )
...
function actionNameFor(action: string): string {
    return (
        getActionInfo(action).label || action || t('quickActions.defaultName')
    )
}
...
<template>
    <div class="space-y-4">
                                    ...
                                    >
                                        {{
                                            item.action?.action.trim()
                                                ? item.action.name
                                                : $t('quickActions.emptyCell')
                                        }}
                                    </span>
                                </span>
                                <span
                                    class="min-w-0 self-stretch truncate font-mono text-[11px] text-(--ui-text-muted)"
                                >
                                    {{ item.action?.action || '—' }}
                                </span>
                            </button>
                    ...
                    <UCard v-if="selectedPage">
                        ...
                        <div v-if="selectedAction" class="space-y-3">
                            ...
                            <UFormField>
                                <template #label>
                                    <FieldLabel
                                        :label="$t('quickActions.nameLabel')"
                                    />
                                </template>
                                <UInput
                            ...
                            <UFormField>
                                <template #label>
                                    <FieldLabel
                                        :label="$t('quickActions.actionLabel')"
                                    />
                                </template>
                                <div class="flex gap-2">
                                    ...
                                    <UButton
                                        ...
                                        square
                                        :aria-label="
                                            $t('quickActions.clearAction')
                                        "
                                        @click="
                                            selectedIndex !== null &&
                                            clearAction(selectedIndex)
                                        "
                                    />
                                </div>
                            </UFormField>
        ...
        <UModal
            ...
            :title="$t('quickActions.confirmDeletePageTitle')"
        >
            <template #body>
                <p class="text-sm">
                    {{ $t('quickActions.confirmDeletePageBody') }}
                </p>
            </template>
            <template #footer>
