...
const gameMode = computed({
    ...
    set: (value) =>
        emit('update:modelValue', { ...props.modelValue, gameMode: value }),
})

const apps = computed<string[]>({
    get: () => props.modelValue.apps ?? [],
    set: (value) =>
        emit('update:modelValue', { ...props.modelValue, apps: value }),
})
...
<template>
    <div class="flex flex-col gap-4">
                ...
                >
                    <template v-if="layoutOptions.length > 0">
                        <UCheckbox
                            ...
                            :label="option.label"
                            :model-value="
                                modelValue.layouts.includes(option.value)
                            "
                            @update:model-value="
                                (checked) => toggleLayout(option.value, checked)
                            "
                        />
                    </template>
        ...
        <UCard v-if="showApps" variant="outline" :ui="{ body: 'p-4' }">
            <AppsListField
                v-model="apps"
