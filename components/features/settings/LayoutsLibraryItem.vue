...
function summarize(
    ...
    const prefix =
        kind === 'whitelist'
            ? t('rules.whitelistPrefix')
            : t('rules.blacklistPrefix')
    if (
        !set ||
        (!set.gameMode &&
            set.layouts.length === 0 &&
            (!set.apps || set.apps.length === 0))
    ) {
        return prefix + '…'
    }
    const parts: string[] = []
...
function entryWhitelistSummary(entryId: string) {
    return summarize(
        'whitelist',
        config.value.settings.layoutConditions[entryId]?.whitelist,
    )
}
...
function entryBlacklistSummary(entryId: string) {
    return summarize(
        'blacklist',
        config.value.settings.layoutConditions[entryId]?.blacklist,
    )
}
...
<template>
    <li
        ...
        :class="[
            layoutMode === 'auto' && !entryIsIncluded(entry.id)
    ...
    >
                ...
                <div class="min-w-0 flex-1">
                    ...
                    >
                        <UTextarea
                            ...
                            :ui="{
                                base: 'min-h-16 resize-none px-3 py-2 text-sm leading-5',
                            }"
                        ...
                        <div class="flex items-center gap-2 flex-wrap">
                            <AppTooltip :text="$t('rules.blacklistHint')">
                                ...
                                >
                                    ...
                                    >
                                        ...
                                        />
                                        <span class="truncate">{{
                                            entryBlacklistSummary(entry.id)
                                        }}</span>
                                    </div>
                                </UButton>
                            </AppTooltip>
                            ...
                            >
                                <div class="flex items-center gap-1 min-w-0">
                                    ...
                                    />
                                    <span class="truncate">{{
                                        entryWhitelistSummary(entry.id)
                                    }}</span>
                                </div>
                            </UButton>
                        </div>
        ...
        <div class="min-w-[12rem] max-w-[16rem] flex flex-col gap-2">
            <div
                ...
                :class="
                    layoutMode === 'auto' ? 'justify-between' : 'justify-end'
                "
            >
                <div
                    v-if="layoutMode === 'auto'"
            ...
            <div
                v-if="
                    layoutMode === 'manual' && manualActiveLayoutId !== entry.id
                "
            ...
            <div
                v-if="layoutMode === 'auto'"
                class="flex flex-col gap-2 items-end justify-end"
                @click.stop
            >
                <AppTooltip :text="$t('rules.autoIncludeHint')">
                    <div class="flex items-center gap-1.5 cursor-pointer">
                        <USwitch
                            ...
                            @update:model-value="
                                entryToggleAuto(entry.id, $event === true)
                            "
                        />
                        <span
