    ...
    >
        ...
        >
            <FieldLabel
                :label="label"
                :hint="hint"
                :hint-visible-on="hintVisibleOn"
            />

            <AppTooltip
                :text="
                    $t('common.usingGlobalDefault', {
                        value: suffix
                            ? `${defaultValue} ${suffix}`
                            : String(defaultValue),
                    })
                ...
                <div class="flex min-w-0 items-center gap-1.5 overflow-hidden">
                    ...
                    >
                        {{ displayValue
                            ...
                            v-if="suffix"
                            class="ml-0.5 font-sans text-[10px] uppercase opacity-60"
                            >{{ suffix }}</span
                        >
                    </span>
                    <UIcon
        ...
        <div
            ...
            @click.stop
        >
            <FieldResetButton :label="$t('common.reset')" @click="reset" />
        </div>
    </div>

    <div
        v-else
        class="-mx-2 flex items-center justify-between rounded-lg px-2 py-1.5"
    >
        <FieldLabel
            :label="label"
            :hint="hint"
