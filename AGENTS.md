...
- `nuxt.config.ts` → `devServer.port = LHC_DEV_PORT || 3010`, `vite.server.strictPort = true`
...
- `src-tauri/tauri.conf.json` → `build.devUrl = "http://localhost:3010"` (used as the documented default; overridden at runtime by `tauri-dev.mjs` when `LHC_DEV_PORT` is set)
