...
export default {
  ...
  settings: {
    ...
    outputPrincipleBody1:
      "Маппер не пытается повторно интерпретировать уже собранные сочетания. Он отправляет системе готовый результат: отдельную клавишу или комбинацию вроде Ctrl+Z, Ctrl+C, Shift+Tab.",
    outputPrincipleBody2:
      "Поэтому приложение обычно получает именно то сочетание, которое вы задали в раскладке. Если действие настроено как Ctrl+Z, система увидит Ctrl+Z, даже если физическая клавиша Z внутри раскладки переназначена на что-то другое.",
  },
  picker: {
